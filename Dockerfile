...
# ENSURE CACHE DIRECTORIES EXIST EARLY (Prevents CI/CD COPY failures)
RUN mkdir -p /root/.cache/cloakbrowser /root/.cloakbrowser /root/.seleniumbase /root/.cache/seleniumbase && \
    touch /root/.cache/cloakbrowser/.keep /root/.cloakbrowser/.keep /root/.seleniumbase/.keep /root/.cache/seleniumbase/.keep
...
RUN rm -rf /opt/ms-playwright/firefox-* \
    ...
    find /root/.cloakbrowser -name "locales" -type d -exec sh -c 'cd "{}" && ls | grep -v -E "en-US|en-GB|fr" | xargs rm -f' \; || true && \
...
# Install ONLY runtime system dependencies + Cleanup
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    # Remove documentation, manuals and locales from the base image
