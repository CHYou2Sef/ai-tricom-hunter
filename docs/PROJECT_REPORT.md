...
1.  **Tier 1 (Patchright 🟦)**: The new default stealth engine. A binary-patched Chromium that bypasses detection at the core protocol level. Optimized for maximum speed and initial transparency.
2.  **Tier 2 (Nodriver 🟢)**: A CDP-only driver with zero WebDriver signals, essentially invisible to fingerprint scanners. Used for high-protection targets like Google SERPs and LinkedIn.
3.  **Tier 3 (Crawl4AI 🟡)**: A managed async scraper built for complex JS rendering and aggressive e-commerce retry logic.
4.  **Tier 4 (Camoufox 🦊)**: The ultimate "Plan Z." A customized Firefox (Gecko) engine. By switching the fundamental browser engine from Chromium to Firefox, it breaks TLS and Network fingerprint patterns designed to trap Chrome-based bots.
...
### D. The Universal Unified Extractor (UUE) Architecture
To completely eliminate "Zero-Token extraction failures" caused by engine-specific DOM evaluation issues (e.g., executing JavaScript inside Nodriver to find JSON-LD), the system decouples Navigation from Extraction.
- **The Process**: The active browser engine simply dumps the raw HTML DOM string. The `UniversalExtractor` (UUE) module natively parses this string offline using `BeautifulSoup`.
- **Tri-Layer Robustness**: It scans the Semantic Layer (Schema.org), Heuristic Layer (hidden `data-attrid` Google patterns), and Visual Layer (`href="tel:"`), guaranteeing absolute data extraction parity across all 4 tiers without consuming expensive LLM tokens.
...
| Metric Topography | Monolithic Script | Current Multi-Agent Model | Phase 11 (4-Tier Stealth) |
| :--- | :--- | :--- | :--- |
| **Data Fidelity Yield** | 45% (High NLP Failure) | > 92% (Expert Mode) | **> 98%** (4-Tier Coverage) |
| **WAF Bypass Rate** | < 10% | 40% | **> 97%** (Patchright/Camoufox) |
| **Concurrency Ceiling** | 1 (Single Process) | Boundless | **Boundless** |
| **Log Integrity** | One-file | Dual-Stream | **Dual-Stream + Alerts** |
| **Human Simulation** | Static `sleep` | Gaussian | **Per-Action Matrix** |
| **Crash Recovery** | Manual Re-alignment | Automatic Chunks | **Adaptive Circuit Breaker** |
| **Engine Redundancy** | None | 3-Tier (Chromium) | **4-Tier (Chromium + Gecko)** |
---
