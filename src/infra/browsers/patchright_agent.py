...
class PatchrightAgent(BaseBrowserAgent):
    ...
    async def _submit_google_search_locked(self, prompt: str) -> bool:
        ...
        try:
            ...
            await page.goto(
                config.GOOGLE_URL, wait_until="load", timeout=self.get_adaptive_timeout_ms(30000)
            )
        ...
        for selector in tab_selectors:
            try:
                ...
                if await tab.count() > 0 and await tab.is_visible(
                    timeout=self.get_adaptive_timeout_ms(1500)
                ):
                    await tab.click()
    ...
    async def _goto_url_locked(self, url: str) -> bool:
        ...
        try:
            await self._page.goto(
                url, wait_until="domcontentloaded", timeout=self.get_adaptive_timeout_ms(15000)
            )
            ...
            for sub in list(set(found_sublinks)):
                try:
                    ...
                    await page.goto(
                        ...
                        timeout=self.get_adaptive_timeout_ms(10000),
                    )
    ...
    async def search_google_ai_mode(self, prompt: str, **kwargs) -> Optional[str]:
        ...
        async with self._lock:
            ...
            try:
                ...
                return await self._wait_for_ai_mode_response_locked(
                    page, timeout_sec=self.get_adaptive_timeout_sec(25)
                )
            except Exception as e:
    ...
    async def _wait_for_ai_mode_response_locked(
        self, page: Page, timeout_sec: int = 25
    ) -> Optional[str]:
        """Internal lock-free wait using smart_wait."""
        ...
        return await wait_for_json_in_text(get_text, timeout=timeout_sec, poll_interval=1.5)
    ...
    async def search_gemini_ai(self, prompt: str, **kwargs) -> Optional[str]:
        ...
        async with self._lock:
            ...
            try:
                ...
                return await self._wait_for_streaming_response_locked(
                    page, GEMINI_RESPONSE_SELECTORS
                )
            except Exception as e:
    ...
    async def _wait_for_streaming_response_locked(
        self, page: Page, selectors: list
    ) -> Optional[str]:
        """Wait for streaming JSON response using smart_wait."""
        from common.smart_wait import wait_for_json_in_text

        async def get_text():
            return await self._extract_first_available_locked(page, selectors)

        return await wait_for_json_in_text(get_text, timeout=60.0, poll_interval=1.0)
    ...
    async def _extract_first_available_locked(self, page: Page, selectors: list) -> Optional[str]:
        ...
        for s in selectors:
            try:
                if not self._page:
                    break
                text = await page.locator(s).first.text_content(
                    timeout=self.get_adaptive_timeout_ms(2000)
                )
