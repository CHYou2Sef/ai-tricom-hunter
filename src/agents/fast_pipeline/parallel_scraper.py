...
class ParallelSocialScraper:
    ...
    def __init__(self, row_index: int, company_name: str, company_address: str = "", siren: str = ""):
    ...
    async def scrape_all(self, urls: Dict[str, List[str]], timeout: float = 30.0) -> List[ScraperResult]:
        """
        Scrape all sources IN PARALLEL using asyncio.gather.
        
        Args:
            urls: {"facebook": [...], "linkedin": [...], "website": [...]}
            timeout: Max seconds for ALL scrapers combined
        
        Returns:
            List of ScraperResult from successful scrapes
        """
        tasks = []
        ...
        # Execute ALL in parallel with shared timeout
        results = await asyncio.wait_for(
            asyncio.gather(*tasks, return_exceptions=True),
            timeout=timeout
        )
        
        # Filter exceptions, collect valid results
        valid_results = []
        for r in results:
    ...
    def _extract_phone(self, text: str, source: str, base_score: int) -> ScraperResult:
        ...
        if candidates:
            ...
            return ScraperResult(
                phone=best["num"],
                score=min(base_score + best.get("score", 0) // 10, 100),
                source=source,
...
def select_best_result(results: List[ScraperResult]) -> Optional[ScraperResult]:
