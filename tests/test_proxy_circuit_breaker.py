...
@pytest.mark.asyncio
async def test_proxy_url_validation(manager):
    ...
    assert manager._validate_proxy_url("http://user:pass@1.1.1.1:1080") is True
    
    # Invalid schemes
    assert manager._validate_proxy_url("ftp://1.2.3.4") is False
    assert manager._validate_proxy_url("socks5://user:pass@1.1.1.1:1080") is False
...
@pytest.mark.asyncio
async def test_proxy_state_machine_transitions(manager):
    addr = "http://1.2.3.4:8080"
    
    # 1. First error -> Still HEALTHY (threshold is 2)
    state = await manager.mark_error(addr)
    ...
    # 4. Fourth error -> BAN (which transitions to QUARANTINE in proxy_manager.py)
    with patch('asyncio.sleep', return_value=None): # Avoid delay in test
        state = await manager.mark_error(addr)
        assert state == ProxyState.QUARANTINE

@pytest.mark.asyncio
async def test_preemptive_rotation_on_warn(manager):
    ...
    with patch('core.config.PROXY_PREEMPTIVE_ROTATE_ON_WARN', True), \
        ...
        await manager.mark_error(addr) # error 1 (healthy)
        state = await manager.mark_error(addr) # error 2 (warn -> triggers ban immediately)
        
        assert state == ProxyState.QUARANTINE
        assert manager._records[addr].state == ProxyState.QUARANTINE

@pytest.mark.asyncio
async def test_force_ban(manager):
    addr = "http://2.2.2.2:8080"
    with patch('asyncio.sleep', return_value=None):
        await manager.mark_banned(addr)
        assert manager._records[addr].state == ProxyState.QUARANTINE
